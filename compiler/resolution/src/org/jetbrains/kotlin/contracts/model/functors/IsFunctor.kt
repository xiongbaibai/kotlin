...
class IsFunctor(constants: ESConstants, val type: KotlinType, val isNegated: Boolean) : AbstractReducingFunctor(constants) {
    ...
    private fun invokeWithValue(value: ESValue, additionalCondition: ESExpression?): List<ConditionalEffect> {
        ...
        val trueResult = ConditionalEffect(trueIs.and(additionalCondition, constants), ESReturns(constants.trueValue))
        val falseResult = ConditionalEffect(falseIs.and(additionalCondition, constants), ESReturns(constants.falseValue))
